<?php
    ...
    $dsn = "mysql:host=localhost;dbname=$room_id;charset=utf8";
        ...
        try {
            ...
            LEFT JOIN users ON topic.user_id = users.user_id LEFT JOIN (SELECT topic_id,CONCAT(comments.id,'/',comments.date,'/',IFNULL(users.name,users.user_id),'/',comments.value) AS datas FROM comments LEFT JOIN users ON comments.user = users.user_id)AS cmt_list ON topic.id = cmt_list.topic_id LEFT JOIN tags ON tags.topic_id = topic.id  GROUP BY topic.id ORDER BY topic.id DESC");
            $stmt -> execute();
